...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        try:
            ...
            messagebox.showinfo("Cantidad de Camiones", f"Se necesitan {int(camiones_necesarios)} camiones para transportar {toneladas} toneladas.")
        except ValueError:
            messagebox.showerror("Error", "Por favor, ingresar una cantidad válida de toneladas.")

    def btn_tiempo_llegada_on_click(self):
        try:
            kilometros = float(self.txt_kilometros.get())
            tiempo_horas = kilometros / 90
            messagebox.showinfo("Tiempo de Llegada", f"Cada camión tardará aproximadamente {tiempo_horas:.2f} horas en llegar.")
        except ValueError:
            messagebox.showerror("Error", "Por favor, ingresar una cantidad válida de kilómetros.")
